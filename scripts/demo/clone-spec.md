...
| H-2 | `new Date().getHours()` | stats-section time-of-day variant (6 variants, §7.4) | **YES, large** (whole section gradient + optionally dark mode at night) | The hwatu clock shim virtualizes `Date.now()`, `new Date()`, and `Date()`. Protocol: launch with `TZ=UTC` and `HWATU_CLOCK_EPOCH_MS=1784872800000` (2026-07-24 06:00 UTC, pre-dawn), then gate at virtual t=0. Never gate across a 5/8/11/16/20/23 hour boundary. NOTE the SSR class is `--time-night`; hydration replaces it, so first-paint-before-JS differs from post-hydration. Diff only after hydration settles. |
